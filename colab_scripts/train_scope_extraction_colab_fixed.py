#!/usr/bin/env python3
"""
Train scope extraction model on Google Colab.
...
def scope_labels_to_text(scope_labels: Dict[str, Any]) -> str:
    """Convert scope_labels dict to text for seq2seq target."""
    if not scope_labels:
        return ""
    
    parts = []
    # Check plural names first (product_types, components, etc.)
    product_types = scope_labels.get("product_types") or scope_labels.get("product_type", [])
    if product_types and (isinstance(product_types, list) and len(product_types) > 0):
        parts.append(f"product_types: {', '.join(product_types)}")
    
    components = scope_labels.get("components") or scope_labels.get("component", [])
    if components and (isinstance(components, list) and len(components) > 0):
        parts.append(f"components: {', '.join(components)}")
    
    materials = scope_labels.get("materials") or scope_labels.get("material", [])
    if materials and (isinstance(materials, list) and len(materials) > 0):
        parts.append(f"materials: {', '.join(materials)}")
    
    processes = scope_labels.get("processes") or scope_labels.get("process", [])
    if processes and (isinstance(processes, list) and len(processes) > 0):
        parts.append(f"processes: {', '.join(processes)}")
    
    thresholds = scope_labels.get("thresholds") or scope_labels.get("threshold", [])
    if thresholds:
        if isinstance(thresholds, list) and len(thresholds) > 0:
            parts.append(f"thresholds: {', '.join(thresholds)}")
        elif isinstance(thresholds, str):
            parts.append(f"thresholds: {thresholds}")
    
    quantities = scope_labels.get("quantities") or scope_labels.get("quantity", [])
    if quantities:
        if isinstance(quantities, list) and len(quantities) > 0:
            parts.append(f"quantities: {', '.join(quantities)}")
...
def records_to_samples(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for rec in records:
        ...
        has_content = False
        for key in ['product_types', 'components', 'materials', 'processes', 'thresholds', 'quantities']:
            val = scope_labels.get(key, [])
            if isinstance(val, list) and len(val) > 0:
                has_content = True
                break
        
        if not has_content:
            continue  # Skip samples without meaningful scope labels
        
        text = rec.get("text", "")
        target = scope_labels_to_text(scope_labels)
...
def main():
    ...
    parser.add_argument("--output", type=str, default="scope_extraction", help="Output directory")
    ...
    print(f"Samples with scope_labels: {len(records)}")
    
    samples = records_to_samples(records)
    non_empty = sum(1 for s in samples if s["target"])
    print(f"Samples with non-empty scope_labels: {non_empty} ({100*non_empty/len(records):.1f}%)")
